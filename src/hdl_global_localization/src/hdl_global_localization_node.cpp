...
public:
  GlobalLocalizationNode() : nh(), private_nh("~") {
    set_engine(private_nh.param<std::string>("global_localization_engine", "FPFH_RANSAC")); //lym设置重定位方法
...
private:
  ...
  bool set_global_map(SetGlobalMapRequest& req, SetGlobalMapResponse& res) {
    ...
    ROS_WARN("global location map size is %d",cloud->points.size());
  ...
  bool query(QueryGlobalLocalizationRequest& req, QueryGlobalLocalizationResponse& res) {
    ...
    ROS_WARN("global location laser size is %d",cloud->points.size());

    auto results = engine->query(cloud, req.max_num_candidates);//lym求解
