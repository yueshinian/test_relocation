...
class FitAllanAcc
{
    ...
    public:
    FitAllanAcc( std::vector< double > sigma2s, std::vector< double > taus, double freq );
    ...
    /**
     * @brief getN
     *          White Noise / White Acceleration Noise, a.k.a. Velocity Random Walk, Noise Density / Acceleration Noise Density
     *          Kalibr: \sigma_g, Gyroscope "white noise", gyroscope_noise_density (see https://github.com/ethz-asl/kalibr/wiki/IMU-Noise-Model)
     *          equivalent units: m / s^2 / sqrt(Hz)
     * @unit: m / (s * sqrt(s))
     * @return
     */
    double getN( ) const;
