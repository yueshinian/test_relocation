...
namespace eskf {
  ...
  private:
    ...
    bool initializeFilter();//初始化状态矩阵X
    void initialiseQuatCovariances(const vec3& rot_vec_var);//初始化P矩阵
    ...
    void makeSymmetrical(scalar_t (&cov_mat)[k_num_states_][k_num_states_], uint8_t first, uint8_t last);//方差矩阵对称
    ...
    void fixCovarianceErrors();//约束方差最大最小值
    void initialiseCovariance();//初始化P矩阵
    void predictCovariance();//更新预测协方差矩阵
